...
#' one exponential term (y = m^2x + ny + b) for nx number of dependent variables and testing every pair of independent
#' variables (nx).
#'
...
model2v4pB<-function(model.data,ny,nx,CV=F,CV_n=1,r2random=F,runs=1000)
{
  ...
  for(i in 1:ny)
  {
    ...
    write.table(aicmejord,paste("Best mod AIC-2var4Bpar",names(model.data)[i+1],"",".xls",sep = ""),sep = "\t",
                row.names = F,col.names = c("Y vs X1 +X1^2 + X2","r2","AICc","F.p.value.x1","F.p.value.x1^2",
                                            "F.p.value.x2","coef[1]","coef[2]","coef[3]","coef[4]"))
  }
  ##---------------------------Cross-validation--------------------------------
  if(CV==T)
  {
    ...
    for(i in 1:ny)
    {
      ...
      for(j in 1:choose(nx,2))
      {
        ...
        means[i,j,] = c(paste(names(model.data)[y],"vs",names(model.data)[x1],"&",paste0(names(model.data)[x1],"^2"),"&",names(model.data)[x2]),mean.r2.CV,mean.r2.data.vs.pred,mean.CV.CV,mean.coefs[1],mean.coefs[2],mean.coefs[3],mean.coefs[4])
      }
