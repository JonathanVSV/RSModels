...
#' (y = mx + ny + ox:y + b) for nx number of dependent variables and testing every pair of independent variables (nx).
#'
    ...
    for(i in 1:ny)
    {
      ...
      for(j in 1:choose(nx,2))
      {
        ...
        means[i,j,] = c(paste(names(model.data)[y],"vs",names(model.data)[x1],"&",names(model.data)[x2]),
                        mean.r2.CV,mean.r2.data.vs.pred,mean.CV.CV,mean.coefs[1],mean.coefs[2],mean.coefs[3],mean.coefs[4])
      }
