...
model1v3p<-function(model.data,ny,nx,CV=F,CV_n=1,r2random=F,runs=1000)
{
  ...
  for(i in 1:ny)
  {
    ...
    write.table(aicmejord,paste("Best mod AIC-1var3par",names(model.data)[i+1],
                                "",".xls",sep = ""),sep = "\t",row.names = F,
                col.names = c("Y vs X1 & X1^2","r2","AICc","F.p.value.x1",
                              "F.p.value.x1^2","coef[1]","coef[2]","coef[3]"))
  }
