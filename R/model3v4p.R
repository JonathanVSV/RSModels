...
#' (y = mx + ny + oz + b) for nx number of dependent variables and testing every pair of independent
#' variables (nx).
#'
...
model3v4p<-function(model.data,ny,nx,CV=F,CV_n=1,r2random=F,runs=1000)
{
  ...
  colnames(mejores)<-c("Atributo","Var imagen 1","Var imagen 2","Var imagen 3","R2","F. p-value 1",
                       ...
                       "Slope x3")
  if(length(results[,,2])==ny)
  {
    for (i in 1:ny)#Cada [i,] es el conjunto de modelos posibles para cada atributo de la comunidad
    {
      mejormo<-1
      mejores[i,1]<-results[,,1][i]
      mejores[i,5]<-max(as.numeric(results[,,2][i]))
      mejores[i,6]<-results[,,4][i]
      mejores[i,7]<-results[,,5][i]
      mejores[i,8]<-results[,,6][i]
      mejores[i,9]<-results[,,3][i]
      mejores[i,10]<-results[,,7][i]
      mejores[i,11]<-results[,,8][i]
      mejores[i,12]<-results[,,9][i]
      mejores[i,13]<-results[,,10][i]
    }
  }else{
    for (i in 1:ny)#Cada [i,] es el conjunto de modelos posibles para cada atributo de la comunidad
    {
  ...
  for(i in 1:ny)
  {
    ...
    write.table(aicmejord,paste("Best mod AIC-3var4par",names(model.data)[i+1],"",".xls",sep = ""),sep = "\t",
                row.names = F,col.names = c("Y vs X1 & X2 & X3","r2","AICc","F.p.value.x1","F.p.value.x2",
                                            "F.p.value.x3","coef[1]","coef[2]","coef[3]","coef[4]"))
  }
  ...
  if(CV==T)
  {
    ...
    for(i in 1:ny)
    {
      ...
      write.table(as.table(means.ord[i,,]),paste("Results-var3par4-CV",names(model.data)[i+1],"",timer,".xls",
                                                 sep = ""),sep = "\t",row.names = F,col.names = c("Y vs X1 & X2 & X3","mean r2.CV","mean r2 data vs pred","mean CV.CV","mean Interc","mean Slope","mean Slope 2","mean Slope 3"))
    }
  }else{
